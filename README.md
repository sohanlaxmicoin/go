...
This repo is the home for all of the public go code produced by SDF.  In addition to various tools and services, this repository is the SDK from which you may develop your own applications that integrate with the rover network.
...
You can use Glide yourself in your project and add rover go as a vendor'd dependency, or you can just drop this repos as `$GOPATH/src/github.com/rover/go` to import it the canonical way (you still need to run `glide install`).
...
* **clients** contains packages that provide client packages to the various Rover services.
* **exp** contains experimental packages.  Use at your own risk.
* **handlers** contains packages that provide pluggable implementors of `http.Handler` that make it easier to incorporate portions of the Rover protocol into your own http server. 
* **support** contains packages that are not intended for consumption outside of Rover's other packages.  Packages that provide common infrastructure for use in our services and tools should go here, such as `db` or `log`. 
...
In addition to the packages described above, this repository contains various packages related to working with the Rover network from a go program.  It's recommended that you use [godoc](https://godoc.org/github.com/rover/go#pkg-subdirectories) to browse the documentation for each.
